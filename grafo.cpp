...
Grafo::Grafo(int numVertices, bool dir)
    : Conjunto(numVertices), V(numVertices), numArestas(0), direcionado(dir) {
    alocarAdj();
    for (int i = 0; i < V; i++) adicionar(i);
}

Grafo::Grafo(const Grafo& outro)
    : Conjunto(outro), V(outro.V), numArestas(outro.numArestas), direcionado(outro.direcionado) {
    alocarAdj();
    for (int v = 0; v < V; v++)
...
bool Grafo::dfsCircuito(int v, int* cor) const {
    return dfsCircuitoAux(v, -1, cor);
}

bool Grafo::dfsCircuitoAux(int v, int pai, int* cor) const {
    cor[v] = 1;
    for (int i = 0; i < adjTam[v]; i++) {
        int u = adj[v][i];
        if (!direcionado && u == pai) continue; // ignora aresta de volta ao pai
        if (cor[u] == 1) return true;
        if (cor[u] == 0 && dfsCircuitoAux(u, v, cor)) return true;
    }
    cor[v] = 2;
    return false;
}
...
// -------------------------------------------------------
// Operadores extras exigidos pelos testes
// -------------------------------------------------------
...
Grafo Grafo::operator|(const Grafo& outro) const {
    if (V != outro.V) throw std::invalid_argument("Grafos com V diferentes");
    Grafo resultado(V, direcionado);
    for (int v = 0; v < V; v++)
        for (int i = 0; i < adjTam[v]; i++)
            resultado.adicionarAresta(v, adj[v][i]);
    for (int v = 0; v < outro.V; v++)
        for (int i = 0; i < outro.adjTam[v]; i++)
            resultado.adicionarAresta(v, outro.adj[v][i]);
    return resultado;
}
...
Grafo Grafo::operator&(const Grafo& outro) const {
    ...
    for (int v = 0; v < V; v++)
        for (int i = 0; i < adjTam[v]; i++)
            if (outro.existeAresta(v, adj[v][i]))
                resultado.adicionarAresta(v, adj[v][i]);
    return resultado;
}
